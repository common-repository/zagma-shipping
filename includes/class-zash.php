...
class ZASH_Core {
	...
	/**
	 * Ensures only one instance of ZASH_Core is loaded or can be loaded.
	 *
	 * @return ZASH_Core
	 * @see ZASH()
	 */
	...
	protected function init_hooks() {
		// Actions
		add_action( 'wp_ajax_zagma_load_cities', [ ZASH_Ajax::class, 'load_cities_callback' ] );
		add_action( 'wp_ajax_nopriv_zagma_load_cities', [ ZASH_Ajax::class, 'load_cities_callback' ] );
		add_action( 'wp_ajax_zagma_load_districts', [ ZASH_Ajax::class, 'load_districts_callback' ] );
		add_action( 'wp_ajax_nopriv_zagma_load_districts', [ ZASH_Ajax::class, 'load_districts_callback' ] );
		...
		add_action( 'woocommerce_admin_field_zash_single_country', [ $this, 'zash_single_country_field' ], 10, 1 );
	...
	public function load_shipping_init() {
		require_once ZASH_DIR . '/methods/zash-method.php';
		require_once ZASH_DIR . '/methods/zagma-method.php';
		require_once ZASH_DIR . '/methods/zagma-sefareshi-method.php';
		require_once ZASH_DIR . '/methods/zagma-pishtaz-method.php';
	}
		...
		foreach ( $types as $type ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( isset( $_POST[ $key ] ) && strlen( $_POST[ $key ] ) ) {
					...
					if ( $field == 'state' ) {
						$name = ZASH()::get_state( $id );
					} else {
						$name = ZASH()::get_city( $id );
					}
	...
	public function zash_single_country_field( $value ) {
	...
	public function enqueue_select2_scripts() {
		...
		wp_register_script( 'zashCheckout', ZASH_URL . 'assets/js/zash.js', [ 'selectWoo' ], '1.0.0' );
	...
	public function get_settings_general( $settings ) {
		...
		foreach ( $settings as &$setting ) {
			...
			if ( $setting['id'] == 'woocommerce_default_country' ) {
				$setting['type'] = 'zash_single_country';
			}
	...
	public function edit_checkout_cities_field( $fields ) {
		...
		foreach ( $types as $type ) {
			...
			$default_state_id = apply_filters( 'zash_default_state', 0, $type );
			...
			$fields[ $type ][ $type . '_city' ] = [
				...
				'required'    => true,
				'id'          => $type . '_city',
				'class'       => apply_filters( 'zash_city_class', $class ),
				'default'     => apply_filters( 'zash_default_city', 0, $type, null ),
				'priority'    => apply_filters( 'zash_city_priority', $fields[ $type ][ $type . '_city' ]['priority'] ),
			];
	...
	public static function states() {
		...
		$states = get_transient( 'zash_states' );
		...
		if ( $states === false ) {
			...
			set_transient( 'zash_states', $states, DAY_IN_SECONDS );
		}
	...
	public static function cities( $state_id ) {
		...
		$cities = get_transient( 'zash_cities_' . $state_id );
		...
		if ( $cities === false ) {
			...
			set_transient( 'zash_cities_' . $state_id, $cities, DAY_IN_SECONDS );
		}
	...
	public function get_terms_option( $term_id ) {
		...
		$options[] = ZASH()->get_term_options( $term_id ) + (array) $term;
	...
	public static function get_option( string $option_name, $default = null ) {
		...
		$options = get_option( 'zash_' . $section, [] );
	...
	public static function set_option( string $option_name, $value ) {
		...
		update_option( 'zash_' . $section, $options );
	}
	...
	public function log( ...$params ) {
		...
		if ( defined( 'WC_LOG_DIR' ) ) {
			$log_file = WC_LOG_DIR . 'zash.log';
		} else {
			$log_file = WP_CONTENT_DIR . '/zash.log';
		}
	...
	public static function zash_sort_state( $a, $b ) {
