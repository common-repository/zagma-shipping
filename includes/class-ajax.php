...
class ZASH_Ajax {
	...
	public static function load_cities_callback() {
		...
		if ( intval( $term_id ) == 0 ) {
			$term_id = apply_filters( 'zash_default_city', 0, $type, $state_id );
		}
	...
	public static function load_districts_callback() {
		...
		$cities = apply_filters( 'zash_districts', $cities, $city_id );
		...
		if ( intval( $term_id ) == 0 ) {
			$term_id = apply_filters( 'zash_default_district', $city_id, $type, $city_id );
		}
